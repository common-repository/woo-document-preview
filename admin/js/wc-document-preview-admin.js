  ...
  $(function () {
    ...
    $("body.post-type-product form#post").on("submit", function () {
      if ($("#wcdp_preview_attachment").val() != "") {
        var fext = $("#wcdp_preview_attachment")
          .val()
          .split(".")
          .pop()
          .toLowerCase();
        if ($.inArray(fext, ["pdf", "doc", "docx", "xlsx", "xls"]) == -1) {
          $(".preview_files p.wcdp-del-msg")
            .text(
              "The file type that you've uploaded is invalid. Please upload given file type documents."
            )
            .show();
          $(
            ".wcdp-document-file .file_url_choose #wcdp_preview_attachment"
          ).addClass("focused");
          $("html, body").animate(
            {
      ...
      if ($("#wcdp_file_urls").val() != "") {
        ...
        if ($.inArray(ext, ["pdf", "doc", "docx", "xlsx", "xls"]) == -1) {
          $(".preview_files p.wcdp-del-msg")
            ...
            .show();
          $(".wcdp-document-file td.file_url #wcdp_file_urls").addClass(
            "focused"
          );
          $("html, body").animate(
            {
