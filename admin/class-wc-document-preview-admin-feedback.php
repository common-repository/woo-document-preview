...
@media (max-width: 767px) {
	...
	.notice.wc-document-preview-notice .wc-document-preview-notice-inner .no-thanks {
			...
			<div class="notice updated wc-document-preview-notice">
				<div class="wc-document-preview-notice-inner">
					<div class="wc-document-preview-notice-icon">
						<img src="<?php echo esc_url( plugin_dir_url( __FILE__ ) ) . 'images/wo_document_preview.png'; ?>" alt="<?php echo esc_attr__( 'Woo Document Preview', 'wc-document-preview' ); ?>" />
					</div>
		...
		public function set_no_bug() {
			...
			if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'wc-document-preview-feedback-nounce' ) || ! is_admin() || ! isset( $_GET[ $this->nobug_option ] ) || ! current_user_can( 'manage_options' ) ) ) {
				return;
			}
