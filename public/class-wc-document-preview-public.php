...
class Wc_Document_Preview_Public {
	...
	public function wcdp_add_preview_field() {
		...
		if ( ! empty( $wcdp_preview ) && empty( $wcdp_documents ) ) {
			...
			$lnk .= '&embedded=true';
			echo '<a class="wcdp-preview-btn thickbox button" href="' . esc_url( $lnk ) . '&TB_iframe=true&width=600&height=550"><div class="product_meta wcdp-preview-btn-div">' . esc_html( $wcdp_preview['name'] ) . '</div></a>';
		}
		if ( ! empty( $wcdp_documents ) ) :
			foreach ( $wcdp_documents['wcdp_file_names'] as $key => $value ) {
				...
				$lnk .= '&embedded=true';
				if ( ! empty( $value ) ) {
					echo '<a class="wcdp-preview-btn thickbox button" href="' . esc_url( $lnk ) . '&TB_iframe=true&width=600&height=550"><div class="product_meta wcdp-preview-btn-div">' . esc_html( $wcdp_documents['wcdp_file_names'][ $key ] ) . '</div></a>';
				}
			}
			endif;
			return true;
	...
	/**
	 * Added thick box for opening doc.
	 *
	 * @return void
	 */
	public function wcdp_add_thickbox() {
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
			global $post;
			if ( is_woocommerce() && is_product() && 'product' === $post->post_type ) {
				add_thickbox();
			}
		}
	}
